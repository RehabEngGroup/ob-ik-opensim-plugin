/* -------------------------------------------------------------------------- *
 * Orientation Based Inverse Kinematics: InverseKinematicsExtendedSolver.cpp  *
 * -------------------------------------------------------------------------- *
...
namespace OpenSim {
  ...
  InverseKinematicsExtendedSolver::InverseKinematicsExtendedSolver(const Model &model,
  ...
  void InverseKinematicsExtendedSolver::setupMarkerGoals(SimTK::State &s) {
  ...
  void InverseKinematicsExtendedSolver::setupOSensorGoals(SimTK::State &s) {
    ...
    if (_orientationSensorAssemblyCondition->getNumOSensors() != 0) {
  ...
  void InverseKinematicsExtendedSolver::updateGoals(const SimTK::State &p)
  {
    ...
    if (hasMarkersFile_ && _markerAssemblyCondition->isInAssembler()) {
      _markersReference.getValues(s, _markerValues);
      _markerAssemblyCondition->moveAllObservations(_markerValues);
    }
    if (hasOSensorsFile_ && _orientationSensorAssemblyCondition->isInAssembler()) {
